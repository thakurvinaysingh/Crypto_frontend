...
import { FiSearch, FiGrid, FiUsers, FiLogOut } from "react-icons/fi";
...
export default function DashboardLayout() {
  ...
  const handleDisconnect = async () => {
    if (window.confirm("Are you sure you want to disconnect and return to home?")) {
      await disconnectAndHome();
    }
  };
  ...
  return (
    <div className="h-screen w-full bg-[#0b1016] text-white overflow-hidden flex flex-col">
      ...
      {/* Floating bottom dock (mobile only) - golden theme */}
      <div
        className="
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 w-full">
            ...
            <button
              ...
              className="
                ...
                rounded-xl bg-white/20 hover:bg-white/30 active:bg-white/35
                transition
              "
          ...
          <button 
            ...
            className="h-10 w-10 grid place-items-center rounded-2xl bg-white/25 hover:bg-white/30 active:bg-white/35 transition shrink-0"
          ...
          {/* Users icon (team) */}
          <button 
            onClick={() => nav("/dashboard/team")} 
            className="h-10 w-10 grid place-items-center rounded-2xl bg-white/15 hover:bg-white/20 active:bg-white/25 transition shrink-0"
            aria-label="Team"
          >
          ...
          <button 
            ...
            className="ml-auto h-10 w-10 grid place-items-center rounded-2xl bg-red-500/20 hover:bg-red-500/30 active:bg-red-500/40 transition shrink-0"
            aria-label="Disconnect and go home"
            title="Disconnect wallet"
          >
            <FiLogOut className="text-red-200" size={18} />
          </button>
        </div>
      </div>
    </div>
